...
export async function getAdminLogs(req: Request, res: Response) {
    try {
        ...
        const take = Number(req.query.amount ?? 50);
        ...
        const [logs, total] = await Promise.all([
            prisma.adminLog.findMany({
                ...
                include: { actor: { select: { userCode: true, name: true } } }
            }),
            prisma.adminLog.count({ where })
        ]);

        res.json({ data: logs, total, page });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: "Internal Server Error" });
    }
...
export async function getSystemLogs(req: Request, res: Response) {
    try {
        ...
        const [logs, total] = await Promise.all([
            prisma.systemLog.findMany({ where, skip, take, orderBy: { createdAt: 'desc' } }),
